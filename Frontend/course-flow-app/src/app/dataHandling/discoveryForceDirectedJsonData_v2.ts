...
// Imports
import { IDiscoveryData, IDiscoveryNodeData, IDiscoveryLinkData } from '../interfaces/discoveryInterfaces';
...
function processDiscoveryJsonData(): IDiscoveryData  {
    ...
    // Discipline nodes
    let disciplineData: IDiscoveryNodeData[] = [];

    // Unit nodes
    let unitData: IDiscoveryNodeData[] = [];
    ...
    for (let index = 0; index < jsonDataAsArray.length; index++) {
        ...
        if (!disciplineData.find((data) => data.id == disciplineCode)){
        ...
        // Add the required data into the unit node data
        unitData.push({
            id: unitObject["code"],
